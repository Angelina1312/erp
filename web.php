...
use App\Http\Controllers\{Auth\AuthController,
    ...
    MailController,
    StockController,
    Template\AppsController,
    Template\AuthenticationController,
    Template\CardsController,
    Template\ChartsController,
    Template\ComponentsController,
    Template\DashboardController,
    Template\ExtensionController,
    Template\FormsController,
    Template\LanguageController,
    Template\MiscellaneousController,
    Template\PageLayoutController,
    Template\PagesController,
    Template\TableController,
    Template\UserInterfaceController
};
...
Route::get('/', [AuthenticationController::class, 'login_cover'])->name('index')->middleware('guest');
...
Route::prefix('auth')->as('auth.')->group(function () {
    Route::middleware('guest')->group(function () {
        ...
        Route::post('attempt', [AuthController::class, 'login'])->name('attempt');
    });
...
Route::middleware('auth')->group(function () {
    Route::prefix('image')->as('image.')->group(function () {
        Route::get('product/slide/{article}/{fileName}', [\App\Http\Controllers\Products\ProductController::class, 'getSlideUri'])->where('fileName', '.*')->name('product.slider');
        Route::get('product/list/{article}/{fileName}', [\App\Http\Controllers\Products\ProductController::class, 'getListUri'])->where('fileName', '.*')->name('product.list');
    });

    Route::prefix('products')->as('products.')->group(function () {
        Route::get('/',[\App\Http\Controllers\Products\ProductController::class,'index'])->name('list');
        Route::get('{product:id}', [\App\Http\Controllers\Products\ProductController::class, 'show'])->name('show');
    });

    Route::prefix('demands')->as('demands.')->group(function () {
        Route::get('/', [\App\Http\Controllers\Demands\DemandController::class, 'index'])->name('list');
        Route::get('{demand:id}', [\App\Http\Controllers\Demands\DemandController::class, 'show'])->name('show');
    });

    Route::prefix('search')->as('search.')->group(function () {
        Route::get('/', function () {
        })->name('index');
        Route::get('compact/{query}', [\App\Http\Controllers\SearchController::class, 'compact'])->name('compact');
    });
    ...
    Route::prefix('notification')->as('notification.')->group(function () {
        ...
        Route::post('create', [\App\Http\Controllers\NotificationController::class, 'create'])->name('create');
        Route::prefix('render')->group(function () {
            Route::get('wrapper', [\App\Http\Controllers\NotificationController::class, 'renderWrapper'])->name('render.wrapper');
        });
        Route::get('dummy', [\App\Http\Controllers\NotificationController::class, 'dummy'])->name('dummy');
    });

    Route::prefix('reports')->group(function (){
        Route::get('revenue-table', \App\Http\Controllers\Reports\RevenueTableController::class)->name('revenue-table');
        Route::get('plans', [\App\Http\Controllers\Reports\PlanController::class, 'index'])->name('plans');
        Route::get('quarter-schedule', [\App\Http\Controllers\Reports\QuarterlyScheduleController::class, 'index'])->name('quarter-schedule');
        Route::get('cash-flow', [\App\Http\Controllers\Reports\CashFlow\CashFlowExcelController::class, 'index'])->name('cash-flow');
        Route::post('generate-cash-flow', [\App\Http\Controllers\Reports\CashFlow\CashFlowExcelController::class, 'generateCashFlow'])->name('generate-cash-flow');
        Route::post('save', [\App\Http\Controllers\Reports\PlanController::class, 'save']);
        Route::prefix('notification')->as('notification.')->group(function() {
            Route::post('generate-excel-income', [\App\Http\Controllers\Reports\ReportExcelController::class, 'generateExcelIncome'])->name('generate-excel-income');
            Route::post('generate-excel-section', [\App\Http\Controllers\Reports\ReportExcelController::class, 'generateExcelPayments'])->name('generate-excel-section');
        });
    });

    Route::prefix('passports')->as('passports.')->group(function (){
        Route::get('generate-qr', [\App\Http\Controllers\Passports\GenerateQrController::class, 'index'])->name('index');
    });

    Route::prefix('vpn')->group(function () {
        Route::resource('keys', \App\Http\Controllers\Vpn\VpnKeyController::class)
            ->only([
                'index',
                'store',
                'destroy'
            ]);
        Route::get('keys-url/{keyUrl}', [\App\Http\Controllers\Vpn\VpnKeyController::class, 'keyUrl'])
            ->name('keys.url');
        Route::get('keys-send', [\App\Http\Controllers\Vpn\VpnKeyController::class, 'keysSend'])
            ->name('keys.send');
    });

    Route::post('preference', \App\Http\Controllers\PreferencesController::class)->name('preference');

    Route::prefix('cashflow')->group(function (){
        Route::resource('/settings/values', \App\Http\Controllers\Reports\CashFlow\Settings\ValueController::class)->only(['index', 'store', 'destroy']);
    });
...
Route::prefix('dashboard')->middleware('auth')->group(function () {
    ...
    Route::resource('/users', \App\Http\Controllers\UserController::class)->only(['index', 'show', 'edit', 'update', 'destroy']);
});

// Маршрут для отправки писем
Route::get('/sendmail', [MailController::class, 'index'])->name('mail.page')->middleware('auth');
Route::post('/sendmail', [MailController::class, 'send'])->name('mail.send')->middleware('auth');

Route::middleware('auth')->prefix('template')->group(function () {
    // Main Page Route
...
Route::prefix('lebersign')->middleware('auth')->group(function (){
    Route::get('create', [\App\Http\Controllers\LebersignController::class, 'index'])->name('lebersign_create');
    Route::post('save', [\App\Http\Controllers\LebersignController::class, 'save'])->name('lebersign_save');
});

Route::prefix('vacations')->middleware('auth')->group(function (){
    Route::get('/', \App\Http\Controllers\VacationsController::class)->name('vacations.index');
    Route::post('save', [\App\Http\Controllers\VacationsController::class, 'save'])->name('vacations.save');
    Route::post('delete', [\App\Http\Controllers\VacationsController::class, 'delete'])->name('vacations.delete');
});

Route::get('/reports/quarter-schedule-office', [\App\Http\Controllers\Reports\QuarterlyScheduleController::class, 'layoutOffice'])->name('quarter-schedule-office');

Route::get('/downloads/price', 'App\Http\Controllers\Products\PriceExcelController')->name('downloads-price');
